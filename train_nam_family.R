# Training script: the NAM-MIDAS family.
#
# Implements the NAM-MIDAS family of models: per-covariate neural
# aggregation networks, an optional per-covariate residual term, and an
# optional factor channel built from the aggregated representations. The
# Ablation, Full, Residual, FA-NAM-MIDAS, and FA-MIDAS variants are all
# expressed through the use_neural_agg, use_residual, and use_factor
# flags on the shared training routines, and a non-additive Deep-MIDAS
# module is also defined.
#
# The run loop trains the factor-augmented variant FA-NAM-MIDAS under
# the shared IPCW-weighted logistic protocol. It uses the same data
# split as train_baseline.R through set.seed(s * iter), so the baseline
# and neural worker files can be joined afterwards by (iter, t_horizon).
# Per-restart trained weights are saved for the post-hoc Procrustes
# alignment.
#
# Default: 10 iterations, horizons t = c(8, 8.5, 9), 30 tasks in total.
# Reads:   period_final_us_nonfin.csv
# Writes:  nam_worker_s<>_t<>_iter<>.rds
#
# Each task runs in its own worker with a small thread budget for the
# torch and BLAS backends.
...
run_one_iteration <- function(iter, data, s, t_horizon, jmax, num_vars, degree,
  ...
  worker_file <- sprintf("nam_worker_s%d_t%s_iter%02d.rds", s, t_horizon, iter)
...
cat(sprintf("Model: FA-NAM-MIDAS\n"))
...
saveRDS(result_para, sprintf("nam_raw_all_s%d.rds", s))
...
for (t_horizon in c(8, 8.5, 9)) {
  ...
  valid_results <- list()
  for (i in seq_len(it)) {
    wf <- sprintf("nam_worker_s%d_t%s_iter%02d.rds", s, t_horizon, i)
    if (file.exists(wf)) {
