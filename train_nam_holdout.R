# Training script: FA-NAM-MIDAS with the case-study firms held out.
#
# Retrains FA-NAM-MIDAS for the case study. It differs from
# train_nam_family.R in two ways:
#
#   1. The latent aggregation width L is locked at 3 in the
#      cross-validation grid so every iteration and restart shares the
#      same R^L feature space, which is the precondition for orthogonal
#      Procrustes alignment of the K shape functions across iterations.
#
#   2. Six case-study firms (AAPL, MSFT, ENRNQ, KODK, CCTYQ, GM) are
#      dropped from the training panel before the train/test split, so
#      their predictions are genuinely out-of-sample. Their covariate
#      rows and metadata are saved separately to case_study_panel.rds
#      for the case-study prediction script (predict_case_study.R).
#
# The baseline worker files are trained on the full panel, so for the
# Procrustes-aligned shape figures and the case-study predictions only
# the FA-NAM-MIDAS fit needs to be re-run on the held-out panel.
#
# Default: 10 iterations, horizons t = c(8, 8.5, 9), 30 tasks in total.
# Reads:   period_final_us_nonfin.csv
#          period_final_us_nonfin_lookup.csv (to identify the holdouts)
# Writes:  nam_holdout_worker_s<>_t<>_iter<>.rds
#          case_study_panel.rds (held-out firms, written once)
...
run_one_iteration <- function(iter, data, s, t_horizon, jmax, num_vars, degree,
  ...
  Sys.setenv(BLIS_NUM_THREADS     = "4")

  log_file <- "nam_holdout_progress.log"
  ...
  set.seed(s * iter + 1000003L)
  torch_manual_seed(s * iter + 1000003L)

  Z_train_list <- extract_lag_groups(X_train, num_vars, jmax)
  Z_test_list  <- extract_lag_groups(X_test, num_vars, jmax)

  cv_res <- NULL
  best_params <- NULL

  log_msg("FA-NAM CV starting (50 configs x 3 folds)")
  cv_factor <- tryCatch({
    cv_nam_midas(Z_train_list, y_train, w_train,
                 K = num_vars, d = jmax, jmax = jmax, nfolds = 3,
                 n_random = n_random_cv,
                 use_neural_agg = TRUE, use_factor = TRUE,
                 W_fixed = w_fin,
                 data_for_auc = train_dataset, t_horizon = t_horizon)
  }, error = function(e) NULL)

  if (is.null(cv_factor) || is.null(cv_factor$best_params)) {
    best_params_factor <- data.frame(lr = 5e-4, L = 3L, H_agg = 16L, n_agg_layers = 1L,
                                     H_pred = 8L, n_pred_layers = 1L,
                                     lambda1 = 0.01, lambda2 = 0.01,
                                     p_dropout = 0.2, p_group_dropout = 0.1,
                                     weight_decay = 1e-4, output_reg = 0.01,
                                     patience = 40L, grad_clip = 1.0,
                                     r = 3L, H_factor = 16L, n_factor_layers = 1L)
  } else {
    best_params_factor <- cv_factor$best_params
  }
  log_msg(sprintf("FA-NAM CV done: L=%d, r=%d, H_f=%d",
                  ifelse(is.null(best_params_factor$L),3,best_params_factor$L),
                  ifelse(is.null(best_params_factor$r),3,best_params_factor$r),
                  ifelse(is.null(best_params_factor$H_factor),16,best_params_factor$H_factor)))

  log_msg("Ensemble FA-NAM starting")
  ens_factor <- tryCatch({
    train_ensemble_nam(Z_train_list, y_train, w_train, Z_test_list,
                       K = num_vars, d = jmax, jmax = jmax, params = best_params_factor,
                       use_neural_agg = TRUE, use_factor = TRUE,
                       W_fixed = w_fin, n_restarts = 2,
                       save_weights = TRUE)
  }, error = function(e) NULL)

  AUC_factor <- NA; AUC_factor_boot <- rep(NA, bootstrap_number)
  factor_ev <- NULL
  if (!is.null(ens_factor)) {
    AUC_factor <- tryCatch({
      ROC_censor_N(data = test_dataset, prediction = ens_factor$predictions, t = t_horizon)$AUC
    }, error = function(e) NA)
    if (!is.na(AUC_factor)) {
      AUC_factor_boot <- ROC_N_bootstrap(data = test_dataset, prediction = ens_factor$predictions,
                                         t = t_horizon, sim_number = bootstrap_number)
    }
    factor_ev <- ens_factor$explained_var
  }

  log_msg(sprintf("DONE: FA=%s",
                  ifelse(is.na(AUC_factor), "NA", sprintf("%.3f", AUC_factor))))

  full_result <- list(
    AUC_factor = round(AUC_factor, 3), AUC_factor_boot = AUC_factor_boot,
    factor_explained_var = factor_ev,
    cv_auc_factor = if (!is.null(cv_factor)) cv_factor$best_cv_auc else NA,
    best_params_factor = best_params_factor,
    fanam_state_dicts  = if (!is.null(ens_factor)) ens_factor$state_dicts  else NULL,
    fanam_pca_matrices = if (!is.null(ens_factor)) ens_factor$pca_matrices else NULL,
    fanam_arch_specs   = if (!is.null(ens_factor)) ens_factor$arch_specs   else NULL,
    diag_factor = if (!is.null(ens_factor)) ens_factor$diagnostics else NULL,
    pca_matrix  = if (!is.null(ens_factor)) ens_factor$pca_matrix else NULL,
    iter = iter,
    t_horizon = t_horizon
  )

  worker_file <- sprintf("nam_holdout_worker_s%d_t%s_iter%02d.rds", s, t_horizon, iter)
  ...
  list(AUC_factor = round(AUC_factor, 3), iter = iter, saved = TRUE)
}
...
LOOKUP_FILE <- if (exists("LOOKUP_FILE")) LOOKUP_FILE else "period_final_us_nonfin_lookup.csv"
if (file.exists(LOOKUP_FILE)) {
  lookup <- read.csv2(LOOKUP_FILE, stringsAsFactors = FALSE)
  holdout_idx <- which(lookup$tic %in% CASE_STUDY_TICKERS)
  cat(sprintf("Case-study holdout: matched %d of %d target tickers\n",
              length(holdout_idx), length(CASE_STUDY_TICKERS)))
  if (length(holdout_idx) > 0) {
    matched_tickers <- lookup$tic[holdout_idx]
    cat(sprintf("  held-out tickers: %s\n",
                paste(matched_tickers, collapse = ", ")))
    case_study_panel <- list(
      covariates = data[holdout_idx, , drop = FALSE],
      lookup     = lookup[holdout_idx, , drop = FALSE]
    )
    saveRDS(case_study_panel, "case_study_panel.rds")
    cat(sprintf("  saved case_study_panel.rds (%d firms x %d cols)\n",
                nrow(case_study_panel$covariates),
                ncol(case_study_panel$covariates)))
    data <- data[-holdout_idx, , drop = FALSE]
    p <- dim(data)[2] - 5; n <- dim(data)[1]
    cat(sprintf("  training panel after holdout: n=%d (was %d)\n",
                n, n + length(holdout_idx)))
  } else {
    cat("  WARNING: no tickers matched, training on full panel\n")
  }
} else {
  cat(sprintf("WARNING: lookup file %s not found; cannot hold out case study firms\n",
              LOOKUP_FILE))
  cat("Training on FULL panel (case study predictions will not be OOS)\n")
}
...
cat(sprintf("Model: FA-NAM-MIDAS (L = 3 locked, %d firms held out)\n",
            length(CASE_STUDY_TICKERS)))
cat(sprintf("CV: %d random configs\n", n_random_cv))
...
cat(sprintf("\n\nAll %d NAM tasks finished after %.2f hours wall-clock\n\n",
            nrow(all_tasks), as.numeric(elapsed)))

saveRDS(result_para, sprintf("nam_holdout_raw_all_s%d.rds", s))
...
for (t_horizon in c(8, 8.5, 9)) {
  ...
  valid_results <- list()
  for (i in seq_len(it)) {
    wf <- sprintf("nam_holdout_worker_s%d_t%s_iter%02d.rds", s, t_horizon, i)
    if (file.exists(wf)) {
      r <- tryCatch(readRDS(wf), error = function(e) NULL)
      if (!is.null(r) && !is.null(r$AUC_factor)) {
  ...
  auc_fa <- sapply(valid_results, function(x) x$AUC_factor)

  cat(sprintf("  Mean FA-NAM AUC = %.3f (sd %.3f)\n",
              mean(auc_fa, na.rm = TRUE), sd(auc_fa, na.rm = TRUE)))

  for (i in seq_along(valid_results)) {
    r <- valid_results[[i]]
    cat(sprintf("  iter %2d: FA=%.3f\n",
                r$iter,
                ifelse(is.na(r$AUC_factor), 0, r$AUC_factor)))
  }
}

cat("\n\n=== Holdout run complete (L = 3 locked, 6 firms held out) ===\n")
cat(sprintf("Total wall-clock: %.2f hours\n", as.numeric(elapsed)))
cat("Next: run predict_case_study.R for out-of-sample distress\n")
cat("probabilities on the held-out firms.\n")
